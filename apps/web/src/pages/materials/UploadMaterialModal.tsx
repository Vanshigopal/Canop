...
export function UploadMaterialModal({ subjects, onClose, onUploaded }: UploadMaterialModalProps) {
  const [title, setTitle] = useState("");
  const [description, setDescription] = useState("");
  const [subjectId, setSubjectId] = useState("");
  const [chapterNumber, setChapterNumber] = useState("");
  const [chapterTitle, setChapterTitle] = useState("");
  const [accessType, setAccessType] = useState<"BATCH" | "SUBJECT" | "INSTITUTE">("BATCH");
  ...
  async function submit(e: React.FormEvent) {
    ...
    setUploading(true);
    try {
      await api.post("/api/v1/materials", fd, {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
      ...
      >
        <div className="flex items-center justify-between p-5 border-b border-border-soft">
          <h2 className="font-display text-lg">Upload Material</h2>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="p-5 space-y-4">
          ...
          <div>
            ...
            <input
              ...
              placeholder="e.g. Chapter 3 — Periodic Table"
            />
          </div>
          ...
          <div>
            ...
            <select
              value={accessType}
              onChange={(e) =>
                setAccessType(e.target.value as "BATCH" | "SUBJECT" | "INSTITUTE")
              }
              className="w-full px-3 py-2 rounded-md border border-border-soft bg-white/92 text-sm"
            >
          ...
          {accessType === "BATCH" && (
            <div>
              ...
              <div className="space-y-1 max-h-40 overflow-y-auto border border-border-soft rounded-md p-2">
                ...
                {batches.length === 0 && (
                  <div className="text-xs text-text-dim">No batches</div>
                )}
              </div>
            </div>
          )}
