...
export function ExportsPage() {
  ...
  const createExport = useMutation({
    mutationFn: () =>
      api
        .post("/api/v1/exports", { reportType, format: exportFormat })
        .then((r) => r.data.data),
    onSuccess: () => {
      setModalOpen(false);
        ...
        <div className="fixed inset-0 bg-black/30 z-50 flex items-center justify-center p-4">
          <div className="glass-panel rounded-xl p-6 w-full max-w-md">
            ...
            <label className="block mb-3">
              <div className="text-xs text-text-muted mb-1">Report type</div>
              <select
                value={reportType}
                onChange={(e) => setReportType(e.target.value)}
                className="w-full px-3 py-2 rounded-lg bg-bg-warm border border-border-soft text-sm"
              >
            ...
            {exportFormat === "PDF" && (
              <p className="text-xs text-text-muted mb-3">
                PDF exports are rendered as printable HTML. Full PDF rendering coming in a
                later release.
              </p>
            )}
